...
def makeSquare(img, filename, scale):
    ...
    if (args.force_max):
        ...
        if (diff_x % 2 == 0 and diff_y % 2 == 0):
            img_sq = cv2.copyMakeBorder(img_sq, int(diff_y / 2), int(diff_y / 2), int(diff_x / 2), int(diff_x / 2),
                                        bType, value=bColor)
        elif (diff_x % 2 == 0):
            img_sq = cv2.copyMakeBorder(img_sq, int(diff_y / 2) + 1, int(diff_y / 2), int(diff_x / 2), int(diff_x / 2),
                                        bType, value=bColor)
        else:
            img_sq = cv2.copyMakeBorder(img_sq, int(diff_y / 2), int(diff_y / 2), int(diff_x / 2) + 1, int(diff_x / 2),
                                        bType, value=bColor)
    elif (h > w):
        # pad left/right
        diff = h - w
...
def helenaStyle(img, filename, scale):
    make_path = args.output_folder + "helena_style-" + str(scale) + "/"
    if not os.path.exists(make_path):
        os.makedirs(make_path)
    ...
    # crop images from bottom up
    while img_ratio > 1:
        file_counter_image = str(counter_image)
        print("fileCounter: " + file_counter_image)
        temp = img_copy[h - w*counter_image:h, 0:w]
        temp_copy = temp.copy()
        crop = temp_copy[0:w, 0:w]
        crop = image_resize(crop, max=scale)
        saveImage(crop, make_path, filename + "-" + file_counter_image)
        if (args.mirror): flipImage(crop, filename + "-" + file_counter_image, make_path)
        if (args.rotate): rotateImage(crop, filename + "-" + file_counter_image, make_path)
        img_ratio -= 1
        counter_image += 1
...
def processImage(img, filename):
    ...
    if args.process_type == "helena_style":
        helenaStyle(img, filename, args.max_size)
